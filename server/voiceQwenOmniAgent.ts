...
export function createVoiceQwenOmniAgentHandler(channelMgr: ChannelManager) {
  return async function voiceAgentFactory(
    ...
    // Bounds for the present-not-summarize model. See plan in
     ...
     *  Takes user request + dispatched message + reply, generates an
     *  answer-shaped response (not a generic summary). See voiceAgent.ts
     *  for full rationale. */
          ...
          body: JSON.stringify({
            ...
            messages: [
              { role: "system", content:
                ...
                "explaining it. Length is whatever the answer requires." },
              { role: "user", content:
    ...
    async function drainAnnouncementQueue(): Promise<void> {
      ...
      try {
        // Full mode: present rawContent using user's request + dispatched
        // message. Replaces the old generic summarizer. Cache the result
        // so the next voice turn can drill in via prompt-resident memory.
    ...
    const unsubscribeBroadcastListener = channelMgr.onAnyBroadcast(
      (project, filename, data) => {
        // Mica announces chat-card replies by default. Per-card mute via
        // muteSet; global disable via MICA_VOICE_AMBIENT=0.
    ...
    const handler: ChannelHandler = {
      ...
      async onData(_clientId, data) {
        ...
        busy = true;
        try {
          // 1. Audio handoff to omni — no Parakeet sidecar hop. The model
          // takes audio directly via OpenAI's `input_audio` content part.
          // We capture the audio bytes and a format hint here; the LLM
          // call site below assembles them into the user message content.
          //
          // Quirk: omni's audio decoder accepts wav/mp3/flac/ogg/m4a per
          // soundfile. MediaRecorder's webm/opus is also accepted but
          // some omni builds prefer wav — we pass whatever the browser
          // sent and let the model side handle it. The `format` field is
          // soundfile's hint, derived from mime.
          let userText = "";
          let audioPart: { data: string; format: string } | null = null;
          if (msg.audioB64) {
            ...
            const fmt = formatByMime[audioMime] || formatByMime[audioMime.split(";")[0]] || "webm";
            audioPart = { data: msg.audioB64, format: fmt };
            // Placeholder transcript so the rest of the pipeline (history,
            // partial-buffering, tool routing) has SOMETHING to anchor to.
          ...
          //
          // For omni audio turns we ALSO skip — the client sends a complete
          // recording at speech-end, no incremental transcription happens
          // here, and our placeholder "[audio]" text would always look
          // incomplete to the heuristic.
...
`,
          ...
          // Multi-modal content shape: omni's user turn carries an array of
          // content parts (input_audio + text) instead of a plain string.
          // System and assistant turns stay as plain strings — vLLM/OpenAI
          // accepts the union per-message.
          type ContentPart =
          ...
          const messages: Message[] = [
            ...
            // Audio from prior turns is NOT replayed (cost prohibitive);
            // history carries the "[audio]" placeholder we stored.
            ...history.map((h) => ({ role: h.role, content: h.raw || h.content })),
          ];

          // Splice in audio for the current turn (the LAST entry, which is
          // the user message we just pushed). The placeholder text "[audio]"
          // sits beside the audio part as a label the model can use to
          // anchor its understanding (some omni builds expect a text part
          // even when audio is present).
          if (audioPart && messages.length > 0) {
            const lastIdx = messages.length - 1;
            if (messages[lastIdx].role === "user") {
              messages[lastIdx] = {
                role: "user",
                content: [
                  { type: "input_audio", input_audio: audioPart },
                  { type: "text", text: "Transcribe and respond to the user's spoken request above. Follow the response grammar in the system prompt — emit <tool> calls and <say> blocks as needed." },
                ],
              };
            }
          }
          ...
          // full rationale. Three checks: explicit dispatch verbs,
          // implicit promises ("let me check"), and agent-name drops.
            ...
            if ((explicitClaim || implicitPromise || nameDrop) && !nameDropIsOffer) {
              ...
              console.log(`[voice-qwen-omni] nameDrop suppressed (offer phrasing detected): ${JSON.stringify(speakable.slice(0, 200))}`);
            }
          }
          if (!speakable) {
