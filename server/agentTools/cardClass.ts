...
export const createClassTool: AgentToolDef<typeof createClassSchema> = {
  ...
  description:
    "Use this to create OR UPDATE a card class at `.mica/card-classes/<name>/` (writes metadata.json + card.html + card.js + card.css). You supply intent (name, badge, dependencies, content, sidecar); the framework picks the directory, validates the metadata schema, and writes a canonical card.js stub when card_js is omitted. Re-call with the same `name` + same `extension` to UPDATE metadata in place — change `badge`, `defaultTitle`, `scripts`, `styles`, `handler`, `primaryFile`, `sidecar` without delete-then-recreate. card.html/card.js/card.css are preserved unless you pass explicit content, so re-calling never clobbers working files. Only changing `extension` is refused (that's a rename, which would orphan existing instance files — delete the class and recreate if you genuinely need a different extension). For card.html/.js/.css edits on their own, use `mica_edit_class_file` instead. To declare a card-class-private sidecar (server.py / server.ts), pass the `sidecar` field AS AN OBJECT (not a JSON string) — see card-class-handbook § Card-class-private sidecars for the schema. Class creation does not go through write_file — that path doesn't enforce the metadata schema or directory shape and produces extension/dirname mismatches that silently render as TXT. **Prerequisites**: a `canvas/<name>-spec.md` must exist with content, AND `skill('card-class-handbook')` must have been invoked this chat session. The tool returns a structured error otherwise; read the `Next:` line and follow it.",
