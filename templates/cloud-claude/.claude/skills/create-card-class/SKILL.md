...
Before designing any non-trivial subcomponent of the card, **invoke the `discover-library` skill** (see `.claude/skills/discover-library/SKILL.md`). The skill handles search → evaluate → verify URL → record-in-spec.md as one canonical procedure. Don't duplicate that procedure here.
...
**File-changed events on spec.md are NOT a build trigger.** If you arrived at create-card-class because a `[File changes detected]` event surfaced a spec.md edit (the user is iterating), you do NOT skip this gate. The user editing the spec is collaborative refinement, not a green light. Do NOT start writing `.mica/card-classes/*` files. Acknowledge the change in chat ("spec.md updated") and either flag inconsistencies, suggest improvements, or post the explicit gate ("Spec looks firm to me — ok to build?"). **The approval gate is human-driven, not file-watcher-driven, and it's a precondition for ALL build entries.**
...
**Why you must not restart:** the agent runs inside the Claude Code SDK process, which is a child of the Mica backend. `scripts/restart.sh` sends SIGTERM to the backend → SDK dies → agent dies → restart's `start.sh` phase never runs because its parent shell was killed too. End state: backend dead, frontend orphaned, no recovery without user intervention. The system blocks `scripts/(stop|restart)\.sh` at the shell layer for exactly this reason.
