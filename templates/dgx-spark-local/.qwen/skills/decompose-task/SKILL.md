---
name: decompose-task
description: Triggers when the work is large enough that decomposition pays AND a sufficient integration contract is writable upfront — both gates of tenet 12 (real architectural seams + integrated whole exceeds parent's working set). The contract is the central deliverable; without it, parallel subagents produce incompatible code and serial dispatch is just slow inline work. Examples that trigger: multi-card-class projects (chat + calendar + todo), multi-module refactors with stable interfaces. Examples that do NOT trigger: a single small card class (use `create-card-class` inline), tightly coupled work where the contract becomes a partial implementation, bug fixes (use `fix-bug`). Planning-shaped asks ("decompose this for me", "plan this build") still trigger so a plan and contract land on canvas. If both this and `fix-bug` could match, prefer `fix-bug`. If both this and `create-card-class` could match for a single card, prefer `create-card-class`.
---
