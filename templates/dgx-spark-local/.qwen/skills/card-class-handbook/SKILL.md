...
**Spec-conformance verifier (`mica_verify_spec_conformance`).** After writing code, the spec frontmatter becomes the contract your code is checked against. Three contracts fire at write time AND on demand: (1) declared `dependencies.umd_scripts` / `dependencies.styles` URLs must appear in `card.html`; undeclared external `<script>` / `<link>` get warned; (2) per-subtask `mechanism` — `"sidecar proxy"` forbids direct external fetches from `card.js` (route through `mica.fetch('mica-internal://card-server/...')`); (3) every URL literal in `card.js` / `card.html` / `card.css` / `server.py` must be cited within 3 lines above by a `// Per <doc-url>` (or `# Per <doc-url>`) comment per discover-dependency § 3c, or opted out with `// mica-skip-cite: <reason>`. Call `mica_verify_spec_conformance({ card_class })` after any batch of code edits — the verdict tag (`CLEAN | WARNINGS | ERRORS`) tells you whether your code matches what the spec promised. A `[WARN] Contract 3` on an OpenSky / OAuth / external-service URL is the signal to go re-read the docs before that URL ships.
